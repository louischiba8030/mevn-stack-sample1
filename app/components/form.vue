<template>
	<div>
		...
		<v-form>
			...
			<v-btn @click="submit({ id, name, email, age, password })">{{
				id ? "Edit" : "Submit"
				}}</v-btn>
		</v-form>
	</div>
...
export default {
	computed: {
		...
		age: {
			...
			set (value) {
				this.$store.commit("user/setAge", value)
			},
		},
		password: {
			get() {
	...
	methods:{
		async submit(user) {
			if(user.id) {
				await this.$axios.put("http://localhost:4341/users/" + user.id, {
					name: user.name,
					email: user.email,
					age: user.age,
				...
				await this.$axios.post("http://localhost:4341/users/", {
					name: user.name,
					email: user.email,
					age: user.age,
					password: user.password
			...
			this.resetUser({ id: 0, name: "", email: "", age: 0, password: "" })
			this.$store.commit(
				"users/storeData",
				(await this.$axios.get("http://localhost:4341/users")).data
			);
		},
		resetUser(user) {
			...
			this.$store.commit("user/setAge", user.age)
